...
class SiswaController {
    ...
    async readAllData(req, res) {
        try {
            let datasiswa = await siswa.find().exec();
            //cek jika data ada
            if (datasiswa.length == 0) {
                return res.status(400).json({ message: "Data siswa tidak ditemukan", data: null });
            } else {
                return res.status(200).json({ message: "Data berhasil ditampilkan", data: datasiswa });
            };
        } catch (error) {
            console.log(error);
    ...
    async readOneData(req, res) {
        try {
            let datasiswa = await siswa.findOne({_id:req.params.id}).exec();
            //cek jika data ada
            if (datasiswa.length == 0) {
                return res.status(400).json({ message: "Data siswa tidak ditemukan", data: null });
            } else {
    ...
    async createData(req, res) {
        try {
            ...
            let createData = await siswa.create(data);
    ...
    async updateData(req, res) {
        try {
            ...
            let updateData = await siswa.findOneAndUpdate(
                { _id: req.params.id },
                data,
                { new: true }
            );
    ...
    async deleteData(req, res) {
        try {
            let deleteData = await siswa.deleteOne({ _id: req.params.id });
